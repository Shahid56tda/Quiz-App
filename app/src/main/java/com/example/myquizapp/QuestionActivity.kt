...
import com.example.myquizapp.setData.score
...
class QuestionActivity : AppCompatActivity() {
    ...
    lateinit var questionList:ArrayList<QuestionData>
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding= ActivityQuestionBinding.inflate(layoutInflater)
        setContentView(binding.root)
        Name=intent.getStringExtra(setData.name)
        val receivedImageUri: Uri = intent.getParcelableExtra("img") ?: Uri.EMPTY
        binding.profileImage.setImageURI(receivedImageUri)

       questionList=setData.getQuestion()
        setQuestions()
        binding.opt1.setOnClickListener {
            selectOptionStyle(binding.opt1,1)
        }
  binding.opt2.setOnClickListener {
            selectOptionStyle(binding.opt2,2)
        }
  binding.opt3.setOnClickListener {
            selectOptionStyle(binding.opt3,3)
        }
  binding.opt4.setOnClickListener {
            selectOptionStyle(binding.opt4,4)
        }
        ...
        binding.submit.setOnClickListener {
            if (selectedOption!=0){
                ...
                when{
                    ...
                    intent.putExtra("img",receivedImageUri)
